...
class MessagesController extends Controller
{
    ...
    public function download($fileName)
    {
        ...
        if (file_exists($path)) {
            ...
            return abort(404, "Desculpe, O arquivo não existe no nosso servidor ou foi apagado!");
        }
    }
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    $error_msg = "Extensão do arquivo não permitida!";
                }
            } else {
    ...
    public function fetch(Request $request)
    {
        ...
        return Response::json([
            'count' => $query->count(),
            'messages' => '<p class="message-hint"><span>Diga \'Oi\' e comece a conversar</span></p>',
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'contacts' => $users->count() > 0 ? $contacts : '<br><p class="message-hint"><span>Sua lista de contatos está vazia</span></p>',
        ], 200);
    }
    ...
    public function getFavorites(Request $request)
    {
        ...
        return Response::json([
            'favorites' => $favorites->count() > 0
                ? $favoritesList
                : '<p class="message-hint"><span>Sua lista de favoritos está vazia</span></p>',
        ], 200);
    }
    ...
    public function search(Request $request)
    {
        ...
        return Response::json([
            'records' => $records->count() > 0
                ? $getRecords
                : '<p class="message-hint"><span>Nada há mostrar.</span></p>',
            'addData' => 'html'
        ], 200);
    }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $msg = "Extensão do arquivo não permitida!";
                    $error = 1;
                }
            } else {
                $msg = "Extensão do arquivo não permitida!";
